...
# MAGIC 
# MAGIC ## Instructions:
# MAGIC 1. Run the notebook against a cluster with Databricks ML Runtime version 11.2.x-cpu, to best re-create the training environment.
...
try:
    model_name = dbutils.jobs.taskValues.get(taskKey    = 'deploy_new_model', \
                                            key        = 'new_model_name', \
                                            debugValue = 'No model')
except ValueError:
    model_name = "model_5_2022-12-02"
...
# MAGIC Run the notebook against a cluster with Databricks ML Runtime version 11.2.x-cpu, to best re-create the training environment.. The cell below downloads the model artifacts associated with your model in the remote registry, which include `conda.yaml` and `requirements.txt` files. In this notebook, `pip` is used to reinstall dependencies by default.
# MAGIC 
...
model_name = "model_5_2022-12-02"
input_table_name = "default.test_data"
output_table_path = "/FileStore/batch-inference/model_5_2022-12-02"
...
# MAGIC ```python
# MAGIC output_table = "" # Example: "ml.batch-inference.model_5_2022-12-02"
...
dbutils.jobs.taskValues.set(key    = 'working', \
                            value  = (len(predict) > 0)
                           )
dbutils.notebook.exit(len(predict))
